pipeline {
    ...
    stages {
        ...
        // 💡 [3교시 핵심] 빌드가 성공하든 실패하든 무조건 실행되는 '마무리 청소 구역'입니다.
    post {
        always {
            echo '빌드가 완료되었습니다. 인프라 공간 확보를 위해 이름 없는 찌꺼기(좀비) 이미지들을 청소합니다...'
            // 이름이 <none>으로 변해버린 안 쓰는 도커 이미지 찌꺼기들을 강제로 싹 날려버리는 치트키 명령어입니다.
